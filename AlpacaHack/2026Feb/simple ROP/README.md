...
簡単なバッファオーバーフローのチャレンジは別の関数を呼び出すだけでしたが，少しレベルアップすると実行ファイル内にある命令片（ガジェット）を組み合わせて新たな命令を作り出すこともできます．先日参加させていただいた `katagaitai CTF` の講義でROPチェーンと言うものを詳しく教えていただきました．その時分からなかったことも含めて，詳しいwriteupを書かせていただいたので，よければご一読ください．

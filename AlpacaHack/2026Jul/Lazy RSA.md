...
$$\begin{align*}
    f(x)&= x^2 - \text{hint}\cdot x + 12345*6789*n \\
        &= x^2 - (12345p + 6789q)x + 12345*6789pq \\
        &= (x - 12345p)(x - 6789q)
\end{align*}$$

この関数の解は $`12345p`$ と  $`6789q`$ であるため、出てきた解に対して $`12345`$ で割った値が整数になる方が $`p`$ であり、もう片方の解を $`6789`$ で割った値が $`q`$ になります。

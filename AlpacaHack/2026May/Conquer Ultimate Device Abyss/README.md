...
と表せます。これは(`b`の`f`行)と(`c`の`g`列)の積の`a`倍を(`e`の`f`行`g`列)の`d`倍に足す操作で、以下のような行列積で表せます。(行列と分かるように大文字で表しています)

$$E = a(BC) + dE$$

ややこしいですが、実際の入力は`(a,b,c,d,e)=(12,A,B,29,C)`なので、これが`flag`と一致する時
